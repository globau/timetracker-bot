...
use constant DEFAULT_TIME_ZONE  => 'PST8PDT';
use constant DEFAULT_WORK_WEEK  => '40';
...
sub register {
    ...
    if (__PACKAGE__->exists($nick)) {
        die "'$nick' is already registered\n";
    }

    $dbh->do(
        "INSERT INTO user(nick, time_zone, work_week, last_status) VALUES(?, ?, ?, ?)",
        undef,
        $nick, DEFAULT_TIME_ZONE, DEFAULT_WORK_WEEK, 'O',
    );
}
...
sub commit {
    ...
    my $values = __PACKAGE__->_load_from_db($self->nick)
        or return;
    return if
        $values->{last_status} eq $self->last_status
        && $values->{time_zone} eq $self->time_zone
        && $values->{work_week} eq $self->work_week;

    my $dbh = TimeTracker::DB->instance;
    $dbh->do(
        "UPDATE user SET time_zone=?, work_week=?, last_status=? WHERE nick=?",
        undef,
        $self->time_zone, $self->work_week, $self->last_status, $self->nick,
    );
}
...
sub log_active {
    ...
    my $existing = $dbh->selectrow_hash(
        "SELECT * FROM active WHERE nick=? AND end_time BETWEEN ? AND ?",
        $nick, $end_a, $end_b,
    );
    ...
    if ($existing) {
        ...
        $dbh->do(
            ...
            undef,
            $datetime->as_sql, $nick, $existing->{start_time},
        ) unless $existing->{end_time} eq $datetime->as_sql;

    } else {
        # start a new range
        $dbh->do(
            "INSERT INTO active(nick, start_time, end_time) VALUES(?, ?, ?)",
            undef,
            $nick, $datetime->as_sql, $datetime->as_sql,
        );
    }
}
